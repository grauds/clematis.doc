...
:::info
`ts-jest` is a Jest transformer with source map support that lets use Jest to test projects 
...
[Jasmine test doubles](https://angular.dev/guide/testing/components-scenarios#provide-service-test-doubles);
...
Jest is configured for all the subprojects and libraries by Nx preset configuration file mentioned
above:
